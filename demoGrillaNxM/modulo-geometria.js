...
function crearGeometria(){ 
    ...
    if (forma=="esfera") {
        ...
        superficie3D=new TuboSenoidal(0.15,0.2,1,2);
    }else if(forma=="plano"){
...
function Esfera(radio){
    this.getPosicion=function(u,v){
        ...
        let x=radio*Math.sin(theta)*Math.cos(fi);
        let y=radio*Math.cos(theta);
        let z=radio*Math.sin(theta)*Math.sin(fi);
        return [x, y, z];
    };
    ...
    this.getNormal=function(u,v){
        ...
        let x=Math.sin(theta)*Math.cos(fi);
        let y=Math.cos(theta);
        let z=Math.sin(theta)*Math.sin(fi);
        return [x, y, z];
    }
...
function TuboSenoidal(amplitud, longitud, radio, altura){
    this.getPosicion=function(u,v){
        ...
        let y = altura*v;
        let x = (radio+amplitud*Math.sin(Math.PI*y/longitud))*Math.sin(theta);
        let z = (radio+amplitud*Math.sin(Math.PI*y/longitud))*Math.cos(theta);
        return [x, y, z];
    };
    ...
    this.getNormal=function(u,v){
        ...
        let n = Math.sqrt(Math.pow(x) + Math.pow(y) + Math.pow(z));
        return [x/n, y/n, z/n];
    }
...
function generarSuperficie(superficie,filas,columnas){
    ...
    for (i=0; i < filas - 1; i++) {
        ...
        for (j=0; j < columnas; j++) {
            ...
            indexBuffer.push(i*columnas+j);// i,j
            indexBuffer.push((i+1)*columnas+j);// i+1,j
            
        }
        console.log("indexBuffer fila: ", indexBuffer);
        // cambio de fila sin cortar el strip (opcion 1 de la diapositiva)
        if (i < (filas - 2)) {
            indexBuffer.push((i+1)*columnas+j-1);
            indexBuffer.push((i+1)*columnas);
        }
        console.log("indexBuffer next: ", indexBuffer);

    }
console.log("indexBuffer completo: ", indexBuffer);
